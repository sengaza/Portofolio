...
export const deviceLanguage = I18n.currentLocale();
...
export const convertPickerOption = (data = [], fieldName) => {
  const tempData = data;
  for (let i = 0; i < data.length; i += 1) {
    tempData[i].name = toTitleCase(tempData[i][fieldName]);
    tempData[i].id = i;
...
export const searchName = (data = [], fieldName) => {
  // const tempData = data;
  for (let i = 0; i < data.length; i += 1) {
...
export const getFirstName = string => _.split(string, ' ', 2).join(' ');
 ...
 * @param {Object} fields - format: Objgiect<Name: {value, type}>
...
export const roundNum = (num = 0) => +`${Math.round(`${num}e+3`)}e-3`;
