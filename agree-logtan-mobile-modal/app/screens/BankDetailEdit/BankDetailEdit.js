...
import styles from './BankDetailEdit.styles';
...
export default class BankDetailEdit extends React.Component {
  ...
  _getParams = () => {
    ...
    this.setState({
      data,
      farmerId,
      farmerBankName: data.farmerBankName,
      farmerCardNumber: data.farmerCardNumber
    });
  };

  _handleBankName = async text => {
    await this.setState({ farmerBankName: text });
    await this._checkEligibility();
  };

  _handleAccoutNumber = async text => {
    await this.setState({ farmerCardNumber: text });
    await this._checkEligibility();
  };

  _renderBankName = () => {
    const { farmerBankName = '' } = this.state;
    return (
      <View style={styles.formBankNameContainer}>
        <BasicTitle
          label={I18n.t('bankName')}
          // maxLength={30}
          onChangeText={this._handleBankName}
          placeholder={I18n.t('placeholders.editBankName')}
  ...
  _renderAccountNumber = () => {
    const { farmerCardNumber = '' } = this.state;
    return (
      <View style={styles.formBankNameContainer}>
        <BasicTitle
          ...
          onChangeText={this._handleAccoutNumber}
          placeholder={I18n.t('placeholders.editAccountNumber')}
  ...
  _onPressSave = async () => {
    const { farmerBankName, farmerCardNumber, farmerId } = this.state;
    const params = {
      farmerBankDetail: {
        farmerBankName,
        farmerCardNumber
  ...
  render() {
    ...
    return (
      <MainScreen loading={loading}>
        ...
        <HeaderDetail title={I18n.t('bankDetail')} onPressClose={() => this._onPressClose()} />
