...
import styles from './FarmerData.styles';
...
export default class FarmerData extends React.Component {
  ...
  _getData = async () => {
    try {
      ...
      const result = await ENDPOINT.getFarmerList();
    ...
    return (
      <View>
        <FarmerCard
          ...
          onPress={this._onPressDetail(farmerId)}
        />
      </View>
    );
  };
  ...
  render() {
    ...
    return (
      <MainScreen loading={loading}>
        ...
        <Header title={I18n.t('farmerData')} back search />
